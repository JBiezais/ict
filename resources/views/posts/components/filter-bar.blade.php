@props(['categories', 'baseUrl', 'currentFilters' => [], 'listSelector' => '[data-posts-list]'])

@php
    $categoryIds = collect($currentFilters['category_ids'] ?? [])
        ->map(fn($v) => (int) $v)
        ->filter()
        ->values()
        ->all();
    ...
    if ($dateFrom && $dateTo) {
        ...
        $dateRangeValue =
            $from && $to ? $from->format('d/m/Y') . ' - ' . $to->format('d/m/Y') : "{$dateFrom} - {$dateTo}";
    } elseif ($dateFrom) {
        $from = \Carbon\Carbon::createFromFormat('Y-m-d', $dateFrom);
...
<div class="flex items-center gap-2 mb-6">
        ...
        <x-slot name="content">
            <form method="GET" action="{{ $baseUrl }}" id="filter-form">
                    ...
                    <div class="space-y-2 mb-4">
                        ...
                        <label
                            ...
                            <input type="checkbox" name="include_uncategorized" value="1"
                                {{ $includeUncategorized ? 'checked' : '' }} onchange="this.form.submit()"
                                class="rounded border-neutral-300 dark:border-zinc-600 text-emerald-600 focus:ring-emerald-500 dark:focus:ring-emerald-400 dark:bg-zinc-800">
                ...
                <input type="text" id="filter-date-range" data-date-range readonly
                    placeholder="{{ __('Select date range') }}" value="{{ $dateRangeValue }}"
    ...
    <div x-data="liveSearch({ baseUrl: {{ Js::from($baseUrl) }}, listSelector: {{ Js::from($listSelector) }}, minLength: 2, debounceMs: 300 })">
        <form x-ref="searchForm" method="GET" action="{{ $baseUrl }}"
            class="flex-1 min-w-0 max-w-xl relative" @submit.prevent="handleSubmit($event)">
            @foreach ($selectedCategoryIds as $id)
                <input type="hidden" name="category_ids[]" value="{{ $id }}">
            @endforeach
            <input type="hidden" name="include_uncategorized" value="{{ $includeUncategorized ? '1' : '0' }}">
            ...
            <input type="hidden" name="sort" value="{{ $sort }}">
            <span
                class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400 dark:text-zinc-500">
