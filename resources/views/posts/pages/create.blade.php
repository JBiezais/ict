<x-app-layout>
    <h2 class="text-2xl font-bold text-neutral-900 dark:text-zinc-100 mb-6">{{ __('Create Post') }}</h2>

    <form method="POST" action="{{ route('my-posts.posts.store') }}"
        ...
        <div class="mt-4">
            ...
            <x-form.textarea name="content" id="content" rows="10"
                class="block mt-1 w-full">{{ old('content') }}</x-form.textarea>
            <x-form.input-error :messages="$errors->get('content')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-form.input-label :value="__('Categories')" />
            <x-form.category-tag-input :categories="$categories" :selected="old('category_uuids', [])" />
