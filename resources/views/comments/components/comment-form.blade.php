@props(['post', 'parentUuid' => null, 'rows' => 3])

<form method="POST" action="{{ route('posts.comments.store', $post) }}" class="mt-3">
    @csrf
    @if ($parentUuid)
        <input type="hidden" name="parent_uuid" value="{{ $parentUuid }}" />
    @endif
    <div>
        <label for="comment-content-{{ $parentUuid ?? 'new' }}" class="sr-only">{{ __('Comment') }}</label>
        <textarea name="content" id="comment-content-{{ $parentUuid ?? 'new' }}" rows="{{ $rows }}" required
    ...
    <div class="mt-2 flex justify-end">
        <x-button.primary-button type="submit" class="!py-1.5 !px-3 !text-sm !normal-case !tracking-normal">
            {{ $parentUuid ? __('Reply') : __('Comment') }}
